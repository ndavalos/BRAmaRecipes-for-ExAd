...
fnc_components_Load = {
	...
	_requiresConcreteMixer	 = getNumber(missionConfigFile >> "CfgCraftingRecipes" >> SelectedRecipe  >> "requiresConcreteMixer");
	...
	//Populate Required
	 if (_requiredInteractionModelGroup != "" || _requiresOcean == 1 || _requiresFire == 1 || _requiresConcreteMixer == 1) then { (_display displayCtrl 85502) lbAdd Format["====== REQUIRES ======"]; };
	...
	if (_requiresConcreteMixer == 1)  then 
	{
		...
		{
			(_display displayCtrl 85502) lbAdd Format["%1 - [MISSING]", "Concrete Mixer"];
			_lbsize = lbSize (_display displayCtrl 85502);
			(_display displayCtrl 85502) lbSetColor [_lbsize-1, [0.918, 0, 0,1]];				
		}
		else
		{
			(_display displayCtrl 85502) lbAdd Format["%1 - [OK]", "Concrete Mixer"];
			_lbsize = lbSize (_display displayCtrl 85502);
			(_display displayCtrl 85502) lbSetColor [_lbsize-1, [0.698, 0.925, 0,1]];			
		};			
	};	
